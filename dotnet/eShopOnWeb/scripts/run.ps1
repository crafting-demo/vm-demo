...
$prev = Import-Clixml -Path (Join-Path 'C:\Users\Administrator' 'sandbox-process.xml')
$prev | Stop-Process -ErrorAction SilentlyContinue
$process = Start-Process "dotnet" -ArgumentList "run --launch-profile Web" -PassThru
$process | Export-Clixml -Path (Join-Path 'C:\Users\Administrator' 'sandbox-process.xml')
